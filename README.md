...
- **詳細ログ機能**: デバッグ情報を実行ファイルと同じフォルダの`debug.log`に記録
...
- **ログ機能**: 詳細なデバッグ情報の記録（実行ファイルと同じフォルダ）
...
### 実行ファイル情報
- **メイン実行ファイル**: `NetworkProfileSwitcher.exe` (151KB)
- **メインアセンブリ**: `NetworkProfileSwitcher.dll` (43KB)
- **依存ライブラリ**: 約1.3MB（合計）
...
3. **ログファイル**: 実行ファイルと同じフォルダの`debug.log`を確認
...
- **ステータスバー追加**: アダプタとプリセットの状態を表示するステータスバーを追加
...
- **ログファイル保存場所の改善**: 実行ファイルと同じフォルダにログを保存
